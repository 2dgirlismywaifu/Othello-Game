...
public class LongNTBotPlayer extends Player {
    public LongNTBotPlayer(int type) {
        super(type);
    }
    @Override
    public String getNextPosition(int[][] boardArray) {
        int[] bestPos = this.heuristic(boardArray);
        System.out.println("Nước đi: " + (char) (bestPos[1] + 97) + (8 - bestPos[0]));
        return bestPos[0] + " " + bestPos[1];
    }

    //Phương thức này sử dụng một hàm đánh giá đơn giản để đếm số quân và số nước đi có thể có của người chơi.
    //Bot sẽ được thiết lập là quân đen hay quân trắng là tuỳ thuộc vào người dùng
    //Phương thức này sẽ trả về danh sách các nước đi có thể đạt được điểm cao nhất và lấy ngẫu nhiên các vị trí đó trong danh sách
    private int[] heuristic(int[][] boardArray) {
    ...
    private int[][] copyBoardArray(int[][] boardArray) {
    ...
    private void flip(int[][] boardArray, int row, int col, int color, int oppColor) {
    ...
    private int evaluate(int[][] boardArray, int color, int oppColor) {
    ...
    private boolean isSameboardArray(int[][] boardArray1, int[][] boardArray2) {
        // Lặp qua tất cả các vị trí trên bảng
        for (int i = 0; i < boardArray1.length; i++) {
            for (int j = 0; j < boardArray1[i].length; j++) {
                // Nếu hai bảng khác nhau, trả về false
                if (boardArray1[i][j] != boardArray2[i][j]) {
                    return false;
                }
            }
        }
        // Nếu cả hai bảng boardArray1 và boardArray2 giống nhau, trả về true
        return true;
    }
}
